<template>
  <v-container>
    <v-row outlined color="blue">
      <v-col cols="12" lg="3">
        <v-card v-for="card in cards" :key="card.id" shaped class="mx-auto overlay" max-width="400">
          <v-img class="white--text align-end" height="200px" :src="link(card.id)">
            <v-card-title>{{ card.title }}</v-card-title>
          </v-img>
          <v-card-subtitle class="pb-0">{{ card.id }}</v-card-subtitle>
          <v-card-text
...
export default {
  ...
  methods: {
    link: function (id) {
      return '../assets/img/cards/img' + id + '.jpg'
    }
  }
}
</script>
